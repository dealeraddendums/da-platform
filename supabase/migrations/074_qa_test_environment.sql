...
--
-- All seed string literals use dollar-quoting ($$ for short strings,
-- $j$ for JSON arrays). This avoids the Supabase SQL Editor pre-parser
-- bug that mishandles two-consecutive-apostrophes escapes inside
...
-- UPSERT: ON CONFLICT (id) DO UPDATE refreshes content for any rows
-- already seeded by 073. test_item_id values are stable so qa_submissions
-- FKs remain valid.
