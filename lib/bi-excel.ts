...
export async function buildBiExcel(report: BiReport): Promise<Buffer> {
  ...
  const rows: [string, string | number][] = [
    ...
    ["Converted — independent", report.trials.convertedIndependent],
    ["Converted — total (incl. group)", report.trials.converted],
    ["Conversion rate (%)", report.trials.conversionRate],
    ["Lost trials — total", report.trials.lost],
    ["Lost trials — independent", report.trials.lostIndependent],
    ["Lost trials — group", report.trials.lostGroup],
    ["", ""],
