...
export function LoginForm({ unified = false }: { unified?: boolean }) {
  ...
  // Unified front door: the identifier can be a 4.0 username (not an email) and
  // 4.0 passwords have no 8-char rule — relax client validation to non-empty and
  // let the server route decide. OTP/passkey stay 5.0-only (keyed off emailValid).
  const idValid = unified ? email.trim().length > 0 : emailValid;
  const pwValid = unified ? password.length >= 1 : password.length >= 6;

  const errors = {
    email: touched.email && !idValid ? (unified ? "Username or email is required" : (email ? "Enter a valid email address" : "Email is required")) : null,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (unified) {
      // Server route decides 4.0 vs 5.0, does the 5.0 password check server-side,
      // and returns a redirect (5.0 /dashboard or a one-time 4.0 login_url).
      // Credentials never leave our origin.
      try {
        const res = await fetch("/api/auth/login", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ username: email.trim(), password, next }),
        });
        const data = await res.json().catch(() => ({})) as { ok?: boolean; redirect?: string; error?: string };
        if (res.ok && data.ok && data.redirect) {
          setSuccess(true);
          window.location.assign(data.redirect); // may be an external one-time 4.0 URL or an internal path
          return;
        }
  ...
  return (
    ...
    >
      {/* Email */}
      <div>
        <label className="lp-label" htmlFor="email">{unified ? "Username or email" : "Work email"}</label>
        <input
          id="email"
          type={unified ? "text" : "email"}
          className={`lp-input${errors.email ? " lp-input-error" : ""}`}
          placeholder={unified ? "Username or you@dealership.com" : "you@dealership.com"}
          value={email}
