...
// Clickable card section label with a right-side chevron — turns a card's
// header into a collapse/expand toggle. Shared by SettingsForm and the
// PrinterNudgeCard / PrintHistorySection cards below.
  ...
  return (
    ...
    >
      <p
        ...
        style={{ color: "var(--text-muted)", letterSpacing: "0.06em", margin: 0 }}
      >
        {label}
      </p>
      <span style={{ color: "var(--text-muted)", fontSize: 12, transition: "transform 150ms", display: "inline-block", transform: collapsed ? "rotate(0deg)" : "rotate(180deg)" }}>
        ▼
      </span>
    </button>
  );
...
export default function SettingsForm({ fixedDealerId, fixedDealerUuid, role, groupId, initialSettings, initialLogoUrl, templatesLocked = false }: Props) {
  ...
  const [settings, setSettings] = useState<Omit<DealerSettingsRow, "dealer_id" | "updated_at">>(
    initialSettings
      ? {
          ...
          always_show_cents: initialSettings.always_show_cents === true,
        }
      : { ...SETTING_DEFAULTS }
  );
  ...
  // Cards load collapsed by default (except Dealer Logo, which has no toggle).
  const [aiCollapsed, setAiCollapsed] = useState(true);
  const [priceDisplayCollapsed, setPriceDisplayCollapsed] = useState(true);
  const [templatesCollapsed, setTemplatesCollapsed] = useState(true);
  const [nudgeCollapsed, setNudgeCollapsed] = useState(true);
  ...
  const fetchSettingsAndTemplates = useCallback(async (id: string) => {
    ...
    if (sJson.data) {
      setSettings({
        ...
        always_show_cents: sJson.data.always_show_cents === true,
      });
    }
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      {!isReadOnly && (
        <>
          ...
          {/* AI Content */}
          <div className="card mb-4" style={{ padding: aiCollapsed ? "14px 20px" : "20px" }}>
            <CollapsibleCardHeader label="AI Content" collapsed={aiCollapsed} onToggle={() => setAiCollapsed((c) => !c)} />
            {!aiCollapsed && (
            <>
          ...
          {/* Price Display */}
          <div className="card mb-4" style={{ padding: priceDisplayCollapsed ? "14px 20px" : "20px" }}>
            <CollapsibleCardHeader label="Price Display" collapsed={priceDisplayCollapsed} onToggle={() => setPriceDisplayCollapsed((c) => !c)} />
            {!priceDisplayCollapsed && (
            <>
            <div className="flex items-center gap-4">
              <span className="text-sm font-medium" style={{ color: "var(--text-secondary)" }}>Always show cents</span>
              <button
                className="px-4 py-2 text-sm font-medium rounded"
                style={{
                  background: !settings.always_show_cents ? "var(--blue)" : "var(--bg-subtle)",
                  color: !settings.always_show_cents ? "#fff" : "var(--text-secondary)",
                  border: "1px solid var(--border)",
                  borderRadius: 4,
                  cursor: "pointer",
                }}
                onClick={() => setSettings((s) => ({ ...s, always_show_cents: false }))}
              >
                Off
              </button>
              <button
                ...
                onClick={() => setSettings((s) => ({ ...s, always_show_cents: true }))}
              >
                On
              </button>
            </div>
            <p className="text-xs mt-3" style={{ color: "var(--text-muted)" }}>
              {settings.always_show_cents
                ? "On: prices show $199.00 — every price on the addendum always includes cents."
                : "Off: whole dollars show as $199, and cents appear only when a price has them."}
            </p>
            </>
            )}
          </div>
      ...
      <div className="card mb-4" style={{ overflow: "hidden" }}>
        <div style={{ padding: templatesCollapsed ? "14px 20px" : "20px 20px 0" }}>
          <CollapsibleCardHeader label={isReadOnly ? "Buyer's Guide" : "Default Templates"} collapsed={templatesCollapsed} onToggle={() => setTemplatesCollapsed((c) => !c)} />
          {!templatesCollapsed && (
          <>
      ...
      {!isReadOnly && (
        <>
          ...
          <PrinterNudgeCard settings={settings} setSettings={setSettings} collapsed={nudgeCollapsed} onToggle={() => setNudgeCollapsed((c) => !c)} />

          {/* Print History */}
          {(role === "dealer_admin" || isAdminPicker) && dealerId && (
            <PrintHistorySection dealerId={dealerId} collapsed={historyCollapsed} onToggle={() => setHistoryCollapsed((c) => !c)} />
          )}
...
function PrinterNudgeCard({
  ...
  return (
    <div className="card mb-6" style={{ padding: collapsed ? "14px 20px" : "20px" }}>
      <CollapsibleCardHeader label="Printer Nudge Margins" collapsed={collapsed} onToggle={onToggle} />
      {!collapsed && (
      <>
...
function PrintHistorySection({ dealerId, collapsed, onToggle }: { dealerId: string; collapsed: boolean; onToggle: () => void }) {
  ...
  return (
    <>
      <div className="card mb-6" style={{ padding: collapsed ? "14px 20px" : "20px" }}>
        <CollapsibleCardHeader label="Print History" collapsed={collapsed} onToggle={onToggle} />
        {!collapsed && (
        <>
        <p className="text-xs mb-4" style={{ color: "var(--text-muted)" }}>
      ...
      {open && (
        ...
        >
          <div style={{ background: "#fff", borderRadius: 6, width: "min(480px, 92vw)", boxShadow: "0 8px 32px rgba(0,0,0,0.18)", overflow: "hidden" }}>
            ...
            <div style={{ padding: "16px 16px 20px" }}>
              <p style={{ fontSize: 13, color: "var(--text-secondary)", lineHeight: 1.6, margin: 0 }}>
                This will reset the print status and clear saved options for all active vehicles in your inventory. Blue printed indicators will return to white and vehicles will reload their default options on next print. Historical data for inactive vehicles is preserved.
              </p>
            </div>
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", padding: "12px 16px", borderTop: "1px solid var(--border)", background: "var(--bg-subtle)" }}>
