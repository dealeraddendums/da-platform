...
function UsersTab({ groupId, isSuperAdmin }: { groupId: string; isSuperAdmin: boolean }) {
  ...
  const [pwUser, setPwUser] = useState<GroupUserProfile | null>(null);
  ...
  const fetchUsers = useCallback(async () => {
    ...
    if (res.ok) {
      const json = await res.json() as { data: GroupUserProfile[]; pendingInvitations?: PendingInvite[] };
      setUsers(json.data ?? []);
  ...
  async function resendInvite(inv: PendingInvite) {
    const res = await fetch(`/api/groups/${groupId}/invitations/${inv.id}`, { method: "POST" });
  ...
  async function revokeInvite(inv: PendingInvite) {
    ...
    const res = await fetch(`/api/groups/${groupId}/invitations/${inv.id}`, { method: "DELETE" });
    if (res.ok) {
  ...
  function openSetPassword(u: GroupUserProfile) {
    setPwUser(u);
    setPwValue("");
    setPwShow(false);
    setPwError(null);
          ...
          {pendingInvites.map((inv) => (
            <div key={inv.id} className="px-5 py-2.5 flex items-center justify-between" style={{ borderTop: "1px solid var(--border)" }}>
              <div className="text-sm">
                ...
                <span style={{ color: "var(--text-muted)" }}> · {inv.email} · {inv.role === "group_admin" ? "Group Admin" : "Group User"}</span>
